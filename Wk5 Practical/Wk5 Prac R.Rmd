...
OSM <- st_read(here::here("Wk5 Practical",
  ...
  #select hotels only
  filter(fclass == 'hotel')
Worldcities <- st_read(here::here("Wk5 Practical",
                                  "data source", 
                                  "World_Cities", 
                                  "World_Cities.shp")) %>%
  st_transform(., 27700)
UK_outline <- st_read(here::here("Wk5 Practical",
                                 "data source", 
                                 "gadm41_GBR_shp", 
                                 "gadm41_GBR_0.shp")) %>%
  st_transform(., 27700)
...
t
```
# Arrange our maps using the grid package
```{r}
library(grid)
